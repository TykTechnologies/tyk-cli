...
- `CLIToWire` — used by `apply` to push to the Dashboard
- `WireToCLI` — used by `get` to pull from the Dashboard into readable YAML
...
Parse YAML → PolicyFile
    │
    ▼
Validate schema + durations (offline)
...
The Dashboard returns policies with both `_id` (the MongoDB document ID, used as the policy's actual identifier) and `id` (usually empty, a legacy field). The CLI uses `_id` (`MID` in Go) as the canonical identifier. When creating a policy, the CLI sets `MID` from `id`.
...
internal/policy/    Pure domain logic (no HTTP, no CLI, no I/O)
  ...
  validate.go       Schema + duration validation, error collection
...
- `internal/policy/convert_test.go` — CLI→wire→CLI round-trip, duration conversion, access rights mapping
...
`TestPolicyIntegration_FullLifecycle` in `policy_test.go` runs the complete lifecycle: list (empty) → apply (create) → list (shows policy) → get (returns CLI schema) → apply (update, idempotent) → delete → list (empty again). This is the acceptance test that proves all commands work together end-to-end.
